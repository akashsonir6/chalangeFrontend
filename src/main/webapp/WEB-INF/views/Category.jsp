...
<div class="container">
<c:url value="/categoryRes" var="abc"/>
<form:form action="${abc}" method="post" commandName="category">
 <legend><center><h2><b><em>Add New Category</em></b></h2></center></legend><br>
 <label for="name">Category name</label>
        <form:input type="text" path="id" class="form-control" placeholder="Enter category id" />
  ...
  <table class="table table-bordered">
    ...
    <c:forEach items="${categorise}" var="cat">
      <tr>
        ...
        <td>${cat.getName()}</td>
        <td>${cat.getCat()}</td>
        <td><a href="<c:url value='/catEdit/${cat.getId()}'/>"class="btn btn-info"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</a>/<a href="<c:url value='/cartDelet/${cat.getId()}'/>" class="btn btn-danger"><i class="fa fa-trash" aria-hidden="true"></i> Delete</a>
        </td>
      </tr>
      </c:forEach>
    </tbody>
